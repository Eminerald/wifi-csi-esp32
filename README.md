...
기존 ESP32(2번) 데이터로는 다음 두 가지 문제가 확인되어, 단순 재해석이 아니라 재캡처(재추출)가 필요하다고 판단함.

sitting_still 표본 손실: 온도 과열로 인해 원본 100개 중 47개(47%)가 유효 CSI 레코드 부족으로 학습에서 제외됨(사람1 24% 스킵, 사람2 70% 스킵). 해당 클래스는 현재 학습/평가 표본이 심하게 부족한 상태.
피험자 일반화 검증 한계: 안테나 개수(1개 vs 2개)가 사람 간 일반화 성능 차이의 원인이라는 가설을 세웠으나, 현재 피험자가 2명뿐이라 통계적으로 확정할 수 없음.
...
두 작업을 동일한 캡처 세션에서 함께 진행하기로 영신님과 합의함(같은 동작을 두 기기로 동시에 촬영해 조건을 통일하려는 목적으로 추정).

아직 정해지지 않은 사항 (확인 필요)

ESP32 재추출 범위: 전체 7클래스 재촬영인지, sitting_still만 재촬영인지
